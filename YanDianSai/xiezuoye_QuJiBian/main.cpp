...
double k1=-0.4127,k2=0.1722,p1=0,p2=0;
double fx=993.85,fy=995.7,cx=501.84,cy=388.83;



cv::Mat left_k=(cv::Mat_<float>(3,3)<<1044,0,567.26,
        0, 1002, 646.68,
        0, 0, 1);

cv::Mat left_dis=(cv::Mat_<float>(1,5)<<-0.7262,1.9964,-2.9099,-0.0342,-0.0218);

cv::Mat right_k=(cv::Mat_<float>(3,3)<<983.8,0,567.26,
        0, 985.117, 646.68,
        0, 0, 1);

cv::Mat right_dis=(cv::Mat_<float >(1,5)<< -0.154, -1.1942, 1.8389, -0.0102, 0.0213);





//double k1=-0.2834,k2=0.0739,p1=0.000193,p2=0.0000176187114;
...
int main()
{
    std::cout<<left_k<<std::endl;
    std::cout<<p2<<std::endl;

    cv::Mat image_before;
    cv::Mat image_after=cv::Mat::zeros(480,640,0);
...
//    cv::waitKey(0);
    image_before=cv::imread("/home/flk/桌面/yds/Sample/right/right_1.jpg",0);///
